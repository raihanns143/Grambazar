...
import { ShoppingCart, Star, Heart, ArrowRight, SlidersHorizontal, ChevronDown } from "lucide-react";
...
export default function CategoryDetails({ params }: { params: { slug: string } }) {
  ...
  const categoryProducts = useMemo(() => {
    let result = products.filter(p => p.categoryId === params.slug && p.price <= priceRange);
    ...
    return (
      <main className="min-h-screen bg-gray-50 flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          ...
          <Link href="/categories" className="text-primary hover:underline font-medium">Browse All Categories</Link>
        </div>
        <Footer />
      </main>
    );
  ...
  return (
    <main className="bg-gray-50/30 min-h-screen">
      ...
      <div className="pt-32 pb-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Category Banner */}
        <div className={`${category.color.split(' ')[0]} rounded-[2.5rem] p-8 md:p-16 mb-12 flex flex-col md:flex-row items-center justify-between gap-8 border border-white/40 shadow-sm relative overflow-hidden`}>
          ...
          <div className="relative z-10 max-w-xl">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-gray-900 mb-6">{category.name}</h1>
            <p className="text-gray-700 text-lg leading-relaxed">{category.description}</p>
          </div>
          <div className="text-[8rem] md:text-[10rem] drop-shadow-2xl relative z-10 leading-none">
            {category.icon}
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
          ...
          <div className="flex items-center gap-4 w-full sm:w-auto">
            <div className="flex items-center gap-3">
              <span className="text-sm font-medium text-gray-500">Max Price: ৳{priceRange}</span>
              <input 
                type="range" min="0" max="1000" step="50"
                value={priceRange} onChange={(e) => setPriceRange(Number(e.target.value))}
                className="w-24 md:w-32 accent-primary h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
              />
            </div>
            ...
            <div className="relative">
              <select 
                ...
                className="appearance-none pl-4 pr-10 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-primary text-sm font-medium text-gray-700 cursor-pointer"
              >
        ...
        {categoryProducts.length === 0 ? (
          <div className="bg-white rounded-3xl border border-gray-100 p-16 text-center shadow-sm">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-2">No items match your filters</h2>
            <p className="text-gray-500 mb-6">Try increasing your price range.</p>
            <button 
